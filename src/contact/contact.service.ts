...
export class ContactService {
  ...
  async create(
    ...
    this.logger.debug(
      `Create new contact for ${user} with data ${JSON.stringify(request)}`,
    );
  ...
  async update(
    ...
    this.logger.debug(
      `Update contact with id ${request.id} for ${user} and data ${JSON.stringify(request)}}`,
    );

    const contactRequest: UpdateContactRequest =
      this.validationService.validate(ContactValidation.UPDATE, request);

    let contact = await this.checkContactExist(
      contactRequest.id,
      user.username,
    );
  ...
  async search(
    ...
    this.logger.debug(
      `Searching contacts for ${user} with data ${JSON.stringify(request)}`,
    );
